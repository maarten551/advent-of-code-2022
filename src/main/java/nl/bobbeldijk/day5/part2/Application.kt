...
class Application : Answerable<String> {
    override fun calculateAnswer(input: MutableList<String>): String {
        val initState = input.takeWhile(String::isNotEmpty).toMutableList()
        ...
        commands.map { Regex("\\d+").findAll(it).map { match -> match.value.toInt() }.toList() }
            ...
            .forEach { command ->
                (0 until command.amount)
                    .map { stacks[command.from].removeLast() }
                    .reversed()
                    .forEach { stacks[command.to].addLast(it) }
            }
